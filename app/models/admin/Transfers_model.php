...
//     public function getPurchaseProductDetails($product_code, $item_batch)
//     {
//         echo "Debugging Input Values:<br>";
//     echo "Product Code: " . $product_code . "<br>";
//     echo "Item Batch: " . $item_batch . "<br>";
//         // print_r($item_batch);
//         // die();
//         // $this->db->save_queries = TRUE;
...
//           $q = $this->db->query('SELECT * FROM `sma_purchase_items` WHERE `product_code` = "'.$product_code.'" AND `batch` = "'.$item_batch.'"');
...
public function getPurchaseProductDetails($product_code, $item_batch)
{
    ...
    // Build and echo the SQL query for debugging
    $query = 'SELECT * FROM `sma_purchase_items` WHERE `product_code` = "'.$product_code.'" AND `batch` = "'.$item_batch.'"';
    ...
    if ($q === false) {
        // echo "Database Error: " . $this->db->error() . "<br>";
...
// die();
    ...
    public function getAllTransferItems($transfer_id, $status)
    {
        if ($status == 'completed') {
            $this->db->select('purchase_items.*, product_variants.name as variant, products.unit, products.hsn_code as hsn_code, products.second_name as second_name')
                ...
                ->where('purchase_items.transfer_id', $transfer_id);
        } else {
    ...
    {
        $q = $this->db->get_where('purchase_items', array('warehouse_id' => $warehouse_id, 'product_id' => $product_id), 1);
        if ($q->num_rows() > 0) {
            return $q->row();
        }
        return FALSE;
