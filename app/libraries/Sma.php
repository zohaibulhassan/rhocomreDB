...
class Sma
{
    ...
    public function formatMoney2($number, $symbol = false)
    {
        if ($symbol !== 'none') { $symbol = $symbol ? $symbol : $this->Settings->symbol; } else { $symbol = null; }
        if ($this->Settings->sac) {
            return ((($this->Settings->display_symbol == 1 || $symbol) && $this->Settings->display_symbol != 2) ? $symbol : '') .
    ...
    public function formatQuantity2($number, $decimals = null)
    {   
        if ($this->Settings->sac) {
            return $this->formatSAC($this->formatDecimal($number, $decimals));
        }
    ...
    public function formatDecimal($number, $decimals = null)
    {
        ...
        // $round_off =  ($number*100);

        // return ( $round_off / 100 ) ;


        // $two_decimal = floor($number * 100) / 100; // this will return 11.54
        ...
        // $cut = substr($number, 0, ( (strpos($number, $sep)+1)+$decimals ));
        // if($ret){ return $cut; }
        // else{ echo $cut; }

        // $this->sma->sigdig($number, '.');

        // return substr($number, 0, $decimals+3);
        return number_format($number, $decimals, '.', '');
    }


    // public function sigdig($num, $sep='.', $sig=2, $ret=FALSE)
    // {
    ...
    public function exd($inv_dates)
    {
        if ($inv_dates) {
            $jsd = $this->dateFormats['js_sdate'];
            if ($jsd == 'dd-mm-yyyy' || $jsd == 'dd/mm/yyyy' || $jsd == 'dd.mm.yyyy') {
                $date = substr($inv_dates, 6, 4) . "-" . substr($inv_dates, 3, 2). "-" . substr($inv_dates, 0, 2);
            } elseif ($jsd == 'mm-dd-yyyy' || $jsd == 'mm/dd/yyyy' || $jsd == 'mm.dd.yyyy') {
                $date = substr($inv_dates, -4) . "-" . substr($inv_dates, 0, 2) . "-" . substr($inv_dates, 3, 2);
            } else {
                $date = $inv_dates;
            }
            return $date;
    ...
    public function fldcalender($ldate)
    {
        if ($ldate) {
            $date = explode(' ', $ldate);
            $jsd = $this->dateFormats['js_sdate'];
            ...
            $time = $date[1];
            if ($jsd == 'dd-mm-yyyy' || $jsd == 'dd/mm/yyyy' || $jsd == 'dd.mm.yyyy') {
                $date = substr($inv_date, 0, 2) . "/" . substr($inv_date, 3, 2) . "/" . substr($inv_date, -4) . " " . $time;
            } elseif ($jsd == 'mm-dd-yyyy' || $jsd == 'mm/dd/yyyy' || $jsd == 'mm.dd.yyyy') {
                $date = substr($inv_date, -4) . "-" . substr($inv_date, 0, 2) . "-" . substr($inv_date, 3, 2) . " " . $time;
            } else {
                $date = $inv_date;
    ...
    public function qrcode($type = 'text', $text = 'http://tecdiary.com', $size = 2, $level = 'H', $sq = null)
    {
        ...
        return "<img src='#' alt='Disable QR Code' class='qrimg' />";
    }
    ...
    public function paid_opts($paid_by = null, $purchase = false, $empty_opt = false)
    {
        ...
        <option value="onlinetransfer"'.($paid_by && $paid_by == 'onlinetransfer' ? ' selected="selected"' : '').'>'.lang("Online Tansfer").'</option>

        
        <option value="payorder"'.($paid_by && $paid_by == 'payorder' ? ' selected="selected"' : '').'>'.lang("Payorder").'</option>
        <option value="withholdingtax"'.($paid_by && $paid_by == 'withholdingtax' ? ' selected="selected"' : '').'>'.lang("With Holding Tax").'</option>

        <option value="retainer"'.($paid_by && $paid_by == 'retainer' ? ' selected="selected"' : '').'>'.lang("Retainer").'</option>
        <option value="balance"'.($paid_by && $paid_by == 'balance' ? ' selected="selected"' : '').'>'.lang("Balance").'</option>
        ...
        <option value="creaditnote"'.($paid_by && $paid_by == 'creaditnote' ? ' selected="selected"' : '').'>'.lang("Creadit Note").'</option>
