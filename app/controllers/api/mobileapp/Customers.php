...
class Customers extends App_Controller
{
    ...
    public function create()
    {
        ...
        if (
            ...
            if ($q->num_rows() > 0) {
                ...
                } elseif ($cdata->phone == $phone) {
                    $this->data['message'] = "Phone Already Available";
                } else {
                    $this->data['testing'] = $cdata;
                    $this->data['q'] = $this->db->last_query();
                }

                $this->data['error_code'] = '004';
            } else {

                $config['upload_path']          = './files/shops/cnic/';
                $config['allowed_types']        = '*';
                $config['encrypt_name'] = true;
                $config['file_name'] = date("Y") . "" . date("m") . "" . date("d") . "" . date("H") . "" . date("i") . "" . date("s");
                $this->load->library('upload', $config);
                if ($this->upload->do_upload('cnicfront')) {
                    $insert['cnicfront'] = $this->upload->file_name;
                }
                $config['upload_path']          = './files/shops/cnic/';
                $config['allowed_types']        = '*';
                $config['encrypt_name'] = true;
                $config['file_name'] = date("Y") . "" . date("m") . "" . date("d") . "" . date("H") . "" . date("i") . "" . date("s");
                $this->load->library('upload', $config);
                if ($this->upload->do_upload('cnicback')) {
                    $insert['cnicback'] = $this->upload->file_name;
                }
    ...
    public function get_order_booker()
    {
        $area_manager = $this->input->post('area_manager');
        if ($area_manager != "") {
            $this->db->select('*');
            $this->db->from('sma_users');
            $this->db->where('area_manager_id', $area_manager);
            $q = $this->db->get();
            if ($q->num_rows() > 0) {
                $this->data['order_bookers'] = $q->result();
                $this->data['code_status'] = true;
                $this->data['message'] = "Success!";
            } else {
                $this->data['message'] = "Invalid Manager!";
                $this->data['error_code'] = '004';
            }
        } else {
            $this->data['message'] = "Required Query Parameter Null!";
