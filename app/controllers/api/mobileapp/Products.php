...
class Products extends App_Controller
{
    ...
    ');
        ...
        $this->db->join('sma_purchase_items as pi', 'pi.product_id = p.id', 'left'); // Join with purchase items
        $this->db->group_by('p.id'); // Group by product id to avoid duplicate rows
        $q = $this->db->get();
