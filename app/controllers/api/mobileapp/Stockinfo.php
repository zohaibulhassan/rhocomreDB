...
class Stockinfo extends App_Controller{
    ...
    function stocks(){
        ...
        $this->db->select('
            ...
            COALESCE((select c.quantity from sma_sale_items as c join sma_sales as d on d.id = c.sale_id where c.product_id = a.product_id and d.customer_id ="'.$customer_id.'" order by c.id DESC limit 1),0) as order_quanity,
        ...
        $this->db->from('sma_sale_items as a');
        $this->db->join('sma_sales as b','b.id = a.sale_id');
