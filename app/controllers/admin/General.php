...
class General extends CI_Controller
{
    ...
    public function etailers()
    {
        $term = $this->input->get('term');
        $this->db->select('id,name as text');
        $this->db->from('etailers');
        if ($term != "") {
            $this->db->like('name', $term);
        }
        $this->db->limit(10);
    ...
    public function searching_products_pos()
    {
        ...
        if ($q->warehouse_id != 0) {
            ...
            $warehouse_id = $this->input->get('branch');
        }
...
public function select_products3()
{
    $sendvalue['codestatus'] = true;
    $id = $this->input->get('id');
    $user_id = $this->session->userdata('user_id');
    ...
    // $warehouse_id = 4;
    // $this->db->select('id,product_id,product_code,product_name,price,dropship,crossdock,mrp,quantity,batch,expiry,quantity_balance');
    // $this->db->from('sma_purchase_items as pi');
    // $this->db->where('product_id', $id);
    // $this->db->where('quantity_balance > 0');
    // $this->db->where('warehouse_id', $warehouse_id);
...
$this->db->where('quantity_balance >', 0);
$this->db->where('warehouse_id', $warehouse_id);
$this->db->group_by('id, product_id, product_code, product_name, price, dropship, crossdock, mrp, quantity, batch, expiry,quantity_balance');
    ...
    $sendvalue['product_name'] = $rows[0]->product_name;
...
//             $html .= '</div>';
//         }
//         // print_r($row->product_code);
//         // die();

//         $sendvalue['html'] = $html;
//         $sendvalue['rows'] = $rows;
//         // $sendvalue['formula'] = $this->db->select('formulas')->from('sma_products as p')->where('id', $id)->get()->row()->formulas;
// // print_r($sendvalue['formula']);
// // die();
//         echo json_encode($sendvalue);
...
//         $product_bar_code = $this->input->post('batch');
//         // $selected_batch_code = $this->input->post('get_selected_batch_code');
//         $remaining_quantity = $this->general_model->getRemainingQuantity($selected_batch_code);   
//         $response['quantity_balance'] = $remaining_quantity;
...
//     $remaining_quantity = $this->general_model->getRemainingQuantity($product_bar_code); 
//     $response['quantity_balance'] = $remaining_quantity;
//     echo json_encode($response);
// }
...
//     $remaining_quantity = $this->general_model->getRemainingQuantity($batch); 
//     $response['quantity_balance'] = $remaining_quantity;
//     echo json_encode($response);
// }
...
public function get_remain_quantity() {
      ...
      $batch_id = $this->input->post('batch_id');
        ...
        $quantity_balance = $this->general_model->get_quantity_balance($batch_id);
        echo json_encode($quantity_balance);
    }
...
//     $response = array('quantity_balance' => $quantity_balance);
//     echo json_encode($response);
// }
