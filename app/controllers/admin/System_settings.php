...
class system_settings extends MY_Controller
{
    ...
    function __construct()
    {
        ...
        if (
            ...
            !$this->GP['updatefurtherdax'] &&
            !$this->GP['package'] &&
            !$this->GP['package_add'] &&
            !$this->GP['package_status'] &&
            !$this->GP['package_delete']
    ...
    public function get_own_companies()
    {
        ...
        $data = array();
        foreach ($rows as $row) {
            $button = '<button class="md-btn md-btn-warning md-btn-wave-light waves-effect waves-button waves-light md-btn-mini editbtn" data-id="' . $row->id . '" data-companyname="' . $row->companyname . '" data-mobile="' . $row->mobile . '" data-rperson="' . $row->registerperson . '" data-ntn="' . $row->ntn . '" data-strn="' . $row->strn . '" data-srb="' . $row->srb . '" data-raddress="' . $row->registeraddress . '" data-waddress="' . $row->warehouseaddress . '" data-autoinvoice="' . $row->auto_invoice_gen . '" data-slip_header="' . $row->slip_header . '" data-slip_footer="' . $row->slip_footer . '" >edit</button>';
            $button .= '<button class="md-btn md-btn-danger md-btn-wave-light waves-effect waves-button waves-light md-btn-mini deletebtn" data-id="' . $row->id . '" >Delete</button>';
            $data[] = array(
                $row->id,
                $row->companyname,
    ...
    public function runquery_own_companies($onlycoun = "no")
    {
        $column_order = array(
            ...
            'own_companies.slip_header',
            'own_companies.slip_footer'
        );
        $column_search = array(
            'own_companies.id',
            'own_companies.companyname',
    ...
    public function insert_own_company()
    {
        ...
        $slipheder = $this->input->post('slipheder');
        $slipfooter = $this->input->post('slipfooter');
        ...
        if ($companyname != "" && $mobile != "") {
            ...
            if ($q->num_rows() > 0) {
                ...
                $insert['slip_header'] = $slipheder;
                $insert['slip_footer'] = $slipfooter;
    ...
    public function update_own_company()
    {
        ...
        if ($companyname != "" && $mobile != "") {
            ...
            $set['mobile'] = $mobile;
            $set['slip_header'] = $slipheder;
            $set['slip_footer'] = $slipfooter;
    ...
    {
        $this->data['error'] = validation_errors() ? validation_errors() : $this->session->flashdata('error');
        $this->db->select('further_tax');
        $query = $this->db->from('sma_settings')->get();
        $this->data['furthertax'] = $query->result();

        $bc = array(array('link' => base_url(), 'page' => lang('home')), array('link' => admin_url('system_settings'), 'page' => lang('system_settings')), array('link' => '#', 'page' => lang('Tax Rates')));
        $meta = array('page_title' => lang('Tax Rates'), 'bc' => $bc);
        $this->page_construct2('settings/futher_tax', $meta, $this->data);
    }


    function updatefurtherdax()
    {
        $furthertax = $this->input->post('furthertax');
        $decimalFurthertax = floatval($furthertax);
        $settingIdToUpdate = 1;
        $data = array(
            'further_tax' => $decimalFurthertax
        );

        // Update the database record
        $this->db->where('setting_id', $settingIdToUpdate);
        $this->db->update('sma_settings', $data);

        redirect('admin/system_settings/futher_tax');
    }
    ...
    function permissions_submit($id = NULL)
    {
        ...
        $data = array(
            ...
            'wallet_view' => $this->input->post('wallet_view'),
            'wallet_add' => $this->input->post('wallet_add'),
            'wallet_edit' => $this->input->post('wallet_edit'),
            'wallet_transations' => $this->input->post('wallet_transations'),

            // 'package' => $this->input->post('package'),
            // 'package' => $this->input->post('package_add')
        );
    ...
    function edit_bulk_discount($id)
    {
        if ($id != "") {
            ...
            if ($q->num_rows() > 0) {
                ...
                $this->data['categories'] = $this->get_categoriesIds(explode(',', $this->data['discount']->category_id));
        ...
        if ($name != "" && $code != "" && $start != "" && $end != "" && $percentage != "") {
            if ($start <= $end) {
                ...
                if ($q->num_rows() > 0) {
                    ...
                    $ucs = array();
                    $categories = $this->get_categoriesIds($categoriesIds);
                    foreach ($categories as $key => $row) {
                        $ucs[] = $row->id;
                        if ($key == 0) {
                            $insert['category_id'] = $row->id;
                            $insert['category_name'] = $row->name;
                        } else {
                            $insert['category_id'] .= ',' . $row->id;
                            $insert['category_name'] .= ',' . $row->name;
                        }
                    ...
                    $insert['type'] = $type;
                    $insert['apply_on'] = $apply_on;
                    $insert['start_date'] = $start;
                    $insert['end_date'] = $end;
                    $insert['created_by'] = $this->session->userdata('user_id');
                    $this->db->insert('bulk_discount', $insert);
                    $discount_id = $this->db->insert_id();
                    if ($apply_on == "all" || $apply_on == "website") {
                        // Create Request
                        $this->create_request('supplier1', $uss, $discount_id);
                        $this->create_request('brand', $ubs, $discount_id);
                        $this->create_request('category_id', $ucs, $discount_id);
                        $this->create_request('id', $ups, $discount_id);
                    }
    ...
    public function update_bulk_discount()
    {
        ...
        if ($name != "" && $code != "" && $start != "" && $end != "" && $percentage != "") {
            if ($start <= $end) {
                ...
                if ($apply_on == "all" || $apply_on == "website") {
                    // Create Request
                    $this->create_request('supplier1', $uss, $id);
                    $this->create_request('brand', $ubs, $id);
                    $this->create_request('category_id', $ucs, $id);
                    $this->create_request('id', $ups, $id);
                }
                $senddata['message'] = "Bulk discount create successfully";
                $senddata['status'] = true;
            } else {
    ...
    public function delete_bulk_discount()
    {
        ...
        if ($this->data['Owner']) {
            if ($reason != "") {
                $sproducts = $this->db->select('*')->from('sma_store_products_tb')->where('discount', $id)->get()->result();
                $this->db->set('discount', '0')
                    ->where('discount', $id)
                    ->update('sma_store_products_tb');
                foreach ($sproducts as $product) {
                    ...
                    $this->db->set('status', 'canceled')
                        ...
                        ->update('sma_store_requests_tb');
                    $sr_insert['product_id'] = $product->product_id;
                    $sr_insert['warehouse_id'] = $warehouse_id;
                    $sr_insert['store_id'] = $store_id;
                    $sr_insert['type'] = "Qty Update";
                    $sr_insert['status'] = 'pending';
                    $this->db->insert('sma_store_requests_tb', $sr_insert);
                }
    ...
    public function create_request($col, $rows, $discount_id)
    {
        if ($col != "" && count($rows) > 0) {
            $products = $this->db->select('id')->from('sma_products')->where_in($col, $rows)->get()->result();
            foreach ($products as $product) {
                $warehouse_id = 4;
                $store_id = 1;
                $this->db->set('discount', $discount_id)
                ...
                $this->db->set('status', 'canceled')
                    ...
                    ->where('store_id', $store_id)
                    ->update('sma_store_requests_tb');
                $sr_insert['product_id'] = $product->id;
                $sr_insert['warehouse_id'] = $warehouse_id;
                $sr_insert['store_id'] = $store_id;
                $sr_insert['type'] = "Qty Update";
                $sr_insert['status'] = 'pending';
    ...
    public function get_categoriesIds($ids)
    {
        if ($ids != "") {
            $this->db->select('id,name');
            $this->db->from('categories');
    ...
    public function package()
    {
        ...
        $this->load->database();
        $query = $this->db->get('sma_package');
        $this->data['rows'] = $query->result();
        $bc = array(array('link' => base_url(), 'page' => lang('home')), array('link' => '#', 'page' => 'Activity Logs'));
        $meta = array('page_title' => 'Package Master', 'bc' => $bc);
        $this->page_construct2('settings/package', $meta, $this->data);
    }
    ...
    public function package_add()
    {
        ...
        $num_months = $post['end_date'];
        $note = $post['note'];
        $end_date = date('Y-m-d', strtotime("+$num_months months", strtotime($start_date)));
        $encry = 'rho' . random_string('alnum', 7);
        ...
        // Insert the data into the 'sma_package' table
        ...
        // }
        redirect(base_url('admin/system_settings/package'));
    }
    ...
    public function package_upgrade()
    {
        ...
        $this->db->update('sma_package'); // Update all rows to set status to 0
        $data = array(
            'name' => $title,
            'start_date' => $start_date,
            'end_date' => $end_date,
        ...
        $this->db->insert('sma_package', $data);
        echo "<h1>Pacakage is upgrade please pay</h1>";
    }
    ...
    public function package_extend($id)
    {
        $this->load->database();

        // Retrieve the package details from the database
        $query = $this->db->get_where('sma_package', array('id' => $id));
        $package = $query->row();

        if ($package) {
            // Calculate the new end date for the extended package
            $startDate = new DateTime(); // Start date is today's date
            $numMonths = $this->input->post('extend_num_months'); // Example: Extend package by 3 months
            $endDate = clone $startDate; // Create a copy of the start date
            $endDate->add(new DateInterval('P' . $numMonths . 'M')); // Add the desired number of months to the end date
            $newEndDate = $endDate->format('Y-m-d');

            // Check if the new end date is greater than the current end date
            if ($newEndDate > $package->end_date) {
                // Update the start date and end date of the package in the database
                $this->db->where('id', $id);
                $this->db->update('sma_package', array(
                ...
                if ($this->db->affected_rows() > 0) {
                    ...
                    // Extension failed
                    echo "Failed to extend the package.";
                }
            } else {
                // Current package end date is already greater
                echo "The current package already has an end date greater than the desired extended end date.";
            }
        } else {
            // Package not found
    ...
    {
        // Load the database and retrieve the package by ID
        $this->load->database();
        $package = $this->db->get_where('sma_package', array('id' => $id))->row();

        // Toggle the status (0 to 1 or 1 to 0)
        $newStatus = $package->status == 0 ? 1 : 0;

        // Update the status in the database
        $this->db->update('sma_package', array('status' => $newStatus), array('id' => $id));

        // Redirect back to the package listing page
        redirect(base_url('admin/system_settings/package'));
    }

    public function package_delete($id)
    {
        // Load the database and delete the package by ID
        $this->load->database();
        $this->db->delete('sma_package', array('id' => $id));
    ...
    public function wallets()
    {
        $this->db->select('
            ...
            GROUP_CONCAT(CONCAT(sma_users.first_name," ",sma_users.last_name)) as users,
        ');
        $this->db->from('sma_wallets');
        $this->db->join('sma_wallet_users', 'sma_wallet_users.wallet_id = sma_wallets.id', 'left');
        $this->db->join('sma_users', 'sma_users.id = sma_wallet_users.user_id', 'left');
        ...
        $this->db->group_by('sma_wallet_users.wallet_id');
        $q = $this->db->get();
        $this->data['wallets'] = $q->result();
        // echo '<pre>';
        // print_r($this->data['wallets']);
        // exit();
        $bc = [['link' => base_url(), 'page' => lang('home')], ['link' => admin_url('settings'), 'page' => lang('settings')], ['link' => '#', 'page' => lang('Wallets')]];
        $meta = ['page_title' => lang('Wallets'), 'bc' => $bc];
        $this->page_construct2('settings/wallets', $meta, $this->data);
    }
    ...
    public function add_wallet()
    {
        ...
        $bc = [['link' => base_url(), 'page' => lang('home')], ['link' => admin_url('settings'), 'page' => lang('settings')], ['link' => '#', 'page' => lang('Add New Wallets')]];
        $meta = ['page_title' => lang('Add New Wallets'), 'bc' => $bc];
        $this->page_construct2('settings/wallets_add', $meta, $this->data);
    }
    ...
    public function insert_wallet()
    {
        ...
        $q = $this->db->select('id')->from('sma_wallets')->where('title', $title)->get();
        if ($q->num_rows() == 0) {
            $insert['title'] = $title;
            $insert['location_id'] = $this->input->post('localtion');
            $insert['first_user'] = 0;
            $insert['second_user'] = 0;
            $this->db->insert('sma_wallets', $insert);
            $wallet_id = $this->db->insert_id();
            $users = $this->input->post('users');
            foreach ($users as $user) {
                $insert2['wallet_id'] = $wallet_id;
                $insert2['user_id'] = $user;
                $insert2['status'] = 1;
                $this->db->insert('sma_wallet_users', $insert2);
            }
            $senddata['status'] = true;
            $senddata['message'] = 'Wallet Add Successfully';
        } else {
            $senddata['message'] = 'Wallet title already available!';
        }
        echo json_encode($senddata);
    }
    ...
    public function delete_wallet()
    {
        ...
        $reason = $this->input->post('reason');
        if ($reason != '') {
            $q = $this->db->select('id')->from('sma_wallet_transations')->where('wallet_id', $id)->get();
            if ($q->num_rows() == 0) {
                $this->db->delete('sma_wallets', ['id' => $id]);
                $this->db->delete('sma_wallet_users', ['wallet_id' => $id]);
                $senddata['status'] = true;
                $senddata['message'] = 'Wallet delete successfully!';
            } else {
                $senddata['message'] = 'You can not delete this wallet';
            }
        } else {
            $senddata['message'] = 'Enter Reason!';
        }
    ...
    public function edit_wallet()
    {
        ...
        $this->data['id'] = $id;
        $this->data['wallet'] = $this->db->select('*')->from('sma_wallets')->where('id', $id)->get()->row();
        $this->data['wallet_users'] = $this->db->select('*')->from('sma_wallet_users')->where('wallet_id', $id)->get()->result();
        $this->data['warehouses'] = $this->settings_model->getAllWarehouses();
        $this->data['userslist'] = $this->settings_model->getAllUsers();
        $bc = [['link' => base_url(), 'page' => lang('home')], ['link' => admin_url('settings'), 'page' => lang('settings')], ['link' => '#', 'page' => lang('Edit Wallets')]];
        $meta = ['page_title' => lang('Edit Wallets'), 'bc' => $bc];
        $this->page_construct2('settings/wallets_edit', $meta, $this->data);
    }
    ...
    public function update_wallet()
    {
        ...
        $title = $this->input->post('title');
        $id = $this->input->post('id');
        $q = $this->db->select('id')->from('sma_wallets')->where('title = "' . $title . '" AND id != ' . $id)->get();
        if ($q->num_rows() == 0) {
            $set['title'] = $title;
            $set['location_id'] = $this->input->post('localtion');
            ...
            $users = $this->input->post('users');
            foreach ($users as $user) {
                $insert2['wallet_id'] = $id;
                $insert2['user_id'] = $user;
                $insert2['status'] = 1;
                $this->db->insert('sma_wallet_users', $insert2);
            }
            $senddata['status'] = true;
            $senddata['message'] = 'Wallet update Successfully';
        } else {
            $senddata['message'] = 'Wallet title already available!';
        }
        echo json_encode($senddata);
    ...
    public function wallets_add_transation()
    {
        ...
        if ($this->data['wid'] != '') {
            $bc = [['link' => base_url(), 'page' => lang('home')], ['link' => admin_url('settings'), 'page' => lang('settings')], ['link' => '#', 'page' => lang('Deposit Wallet')]];
            $meta = ['page_title' => lang('Deposit Wallet'), 'bc' => $bc];
            $this->page_construct2('settings/wallets_add_transation', $meta, $this->data);
        } else {
    ...
    public function wallet_depoit()
    {
        ...
        $insert['created_at'] = $this->input->post('date');
        $insert['account_head'] = $this->input->post('account_head');
        $insert['type'] = '0';
        ...
        if ($this->data['wid'] != "") {
            ...
            $query = $this->db->query("
                ...
                FROM (
                    SELECT
                        w.id AS tid,
                        w.created_at AS tdate,
                        '' AS category,
                        w.account_head as detail,
                        w.note AS particular,
                        w.amount AS debit,
                        0 AS credit,
                        0 AS balance,
                        'Cash-In' AS tstatus
                    FROM sma_wallet_transations AS w 
                    WHERE w.wallet_id = " . $wid . " AND w.type = 0
                    UNION
                    SELECT
                        e.id AS tid,
                        e.date AS tdate,
                        ec.name AS category,
                        e.reference AS detail,
                        e.note AS particular,
                        0 AS debit,
                        e.amount AS credit,
                        0 AS balance,
                        'Cash-Out' AS tstatus
                    FROM sma_expenses AS e
                    LEFT JOIN sma_expense_categories AS ec ON ec.id = e.category_id
            ...
            $bc = array(array('link' => base_url(), 'page' => lang('home')), array('link' => admin_url('settings'), 'page' => lang('settings')), array('link' => '#', 'page' => lang('Wallet Ledger')));
            $meta = array('page_title' => lang('Wallet Ledger'), 'bc' => $bc);
            $this->page_construct2('settings/wallets_transations', $meta, $this->data);
        } else {
            redirect(base_url('admin/system_settings/wallets'));
        }
    }
