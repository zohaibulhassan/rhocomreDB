...
class Purchases extends MY_Controller
{
    ...
    public function add_expense()
    {
        ...
        $bc = [['link' => base_url(), 'page' => lang('home')], ['link' => admin_url('system_settings/add_expense'), 'page' => lang('Add Expense')], ['link' => '#', 'page' => lang('Add Expense')]];
        $meta = ['page_title' => lang('Add Expense'), 'bc' => $bc];
    ...
    public function edit_expense($id = null)
    {
        ...
        if ($id != '') {
            ...
            if ($q->num_rows() > 0) {
                ...
                $bc = [['link' => base_url(), 'page' => lang('home')], ['link' => admin_url('system_settings/edit_expense'), 'page' => lang('Edit Expense')], ['link' => '#', 'page' => lang('Edit Expense')]];
                $meta = ['page_title' => lang('Edit Expense'), 'bc' => $bc];
    ...
    public function get_items()
    {
        ...
        foreach ($rows as $row) {
            ...
            if ($row->tax_type == 1 && $row->tax_rate != 0) {
                $tax_type = 'GST';
            } elseif ($row->tax_type == 2 && $row->tax_rate != 0) {
    ...
    public function add()
    {
        ...
        $this->data['suppliers'] = $this->general_model->GetAllSuppliers();
        $this->data['warehouses'] = $this->general_model->GetAllWarehouses();
        $bc = [['link' => base_url(), 'page' => lang('home')], ['link' => '#', 'page' => 'Purchases']];
        $meta = ['page_title' => 'Purchases', 'bc' => $bc];
        ...
        if (isset($_FILES['itemsfile']) && $_FILES['itemsfile'] != '') {
            if ($_FILES['itemsfile']['type'] == 'text/csv') {
                ...
                if ($this->upload->do_upload('itemsfile')) {
                    ...
                    $keys = ['product_id', 'qty', 'batch', 'expiry_date', 'discount'];
                    $finals = [];
    ...
    public function delete_item()
    {
        ...
        if ($q->num_rows() > 0) {
            ...
            if ($item->quantity == $item->quantity_balance) {
                $this->db->delete('purchase_items', ['id' => $pi_id]);
    ...
    public function purchase_return($id)
    {
        ...
        $bc = [['link' => base_url(), 'page' => lang('home')], ['link' => admin_url('purchases'), 'page' => lang('purchases')], ['link' => '#', 'page' => 'Purchase Return']];
            ...
            if ($item_id[$key] != '' && $return_qty[$key] > 0 && $reason[$key] != '') {
                ...
                if ($item_id[$key] != '') {
                    if ($return_qty[$key] > 0 && $reason[$key] == '') {
                        ...
                        echo json_encode($sendvalue);
                        exit;
                    } elseif ($return_qty[$key] == 0 && $reason[$key] != '') {
    ...
    public function returns()
    {
        ...
        $bc = [['link' => base_url(), 'page' => lang('home')], ['link' => '#', 'page' => 'Purchase Returns']];
        $meta = ['page_title' => 'Purchase Returns', 'bc' => $bc];
    ...
    public function return_delete()
    {
        ...
        if ($reason != '') {
            ...
            foreach ($return_rows as $row) {
                $this->db->delete('sma_purchase_return_items_tb', ['id' => $row->id]);
                $this->purchases_model->updateQty($row->product_id, $row->warehouse_id, $row->purchase_item_id, $row->quantity, '+');
            }
            $this->db->delete('sma_purchase_return_tb', ['purchase_id' => $id]);
    ...
    public function adjustment()
    {
        ...
        $bc = [['link' => base_url(), 'page' => lang('home')], ['link' => admin_url('purchases'), 'page' => lang('purchases')], ['link' => '#', 'page' => 'Batch Adjustment']];
        $meta = ['page_title' => 'Batch Adjustment', 'bc' => $bc];
        $this->page_construct2('purchases/adjustments', $meta, $this->data);
        ...
        if ($reason != '') {
            ...
            if ($q->num_rows() > 0) {
                ...
                if ($data->quantity == $data->quantity_balance) {
                    $this->db->delete('sma_purchase_items', ['id' => $data->id]);
