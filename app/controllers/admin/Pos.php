...
class Pos extends MY_Controller{
    ...
    public function index(){
        ...
        if($rq->num_rows() == 0){
            ...
            $wid = $this->pos_model->getWarehouseIdbyUser();                                                
            $whn = $this->db->select()
                        ->from('warehouses')
                        ->where('id', $wid[0]->warehouse_id)
                        ->get();
            $this->data['whn'] =  $whn->result();
            

            $this->data['register'] = $rq->result()[0];
            $this->data['bulkdiscounts'] = $this->pos_model->bulkdiscounts();
            $this->data['warehouse_id'] = 4;
            /* echo $this->data['warehouse_id'];
            exit(); */
    ...
    public function productlist(){
        ...
        exit; */
        $html = "";
        $this->db->select('a.id,a.name,a.mrp,a.code,b.quantity');        
        $this->db->from('products as a');  
        $this->db->join('purchase_items as b','a.id = b.product_id');   
        $this->db->group_by('a.id'); 
        if($type == "category"){
            if($category != ""){
                $this->db->where('a.category_id = '.$category.' OR a.subcategory_id = '.$category);
            }
        }
        else if($type == "brand"){
            if($brand != ""){
                $this->db->where('a.brand = '.$brand);
            }
        }
        if(!empty($branch)){
                $this->db->where('b.warehouse_id', $branch);
        }
        if($category == "" && $brand == ""){
            $this->db->limit(100);
        }
        ...
        if($uq->num_rows() > 0){
            ...
            if($customer == ""){
                ...
                if($cq->num_rows() == 0){
                    ...
                    $q = $this->db->select()->from('users')->where('id', $user_id)->get()->row();        
                    if($q->warehouse_id != 0){
                        $insert['warehouse_id'] = $q->warehouse_id;
                    }
                    else{
                        $insert['warehouse_id'] = $this->input->post('branches');
                    }
    ...
    public function registerclose_detail_new(){
        $this->data['error']           = (validation_errors() ? validation_errors() : $this->session->flashdata('error'));
        $user_id = $this->session->userdata('user_id');
        $user_register                    = $user_id ? $this->pos_model->registerData($user_id) : null;
        // $register_open_time               = $this->session->userdata('register_open_time');
        $register_open_time               = $user_register ? $user_register->date : null;
        $this->data['register_open_time'] = $user_register ? $register_open_time : null;
        $this->data['cash_in_hand']       = $user_register ? $user_register->cash_in_hand : 0;
        $this->data['users']           = $this->pos_model->getUsers($user_id);
        $this->data['suspended_bills'] = $this->pos_model->getSuspendedsales($user_id);
        $this->data['user_id']         = $user_id;
        $this->db->select('
        ...
        $this->db->where('payments.created_by',$user_register->user_id);
        $this->db->where('payments.date >= ',$user_register->date);
        ...
        $this->db->group_by('payments.paid_by');
        $pq = $this->db->get();
        $this->data['refund'] = $pq->result();
        $this->db->select('SUM(paid) AS total_paid ,SUM(grand_total) AS total_sale');
        $this->db->from('sales');
        $this->db->where('created_by',$user_register->user_id);
        $this->db->where('date >= ',$user_register->date);
        $sq = $this->db->get();
        $this->data['sales'] = $sq->row();
        $this->db->select('SUM(return.grand_total) as total');
        $this->db->from('sma_sale_returns_tb as return');
        $this->db->join('sales','sales.id = return.sale_id','left');
        $this->db->where('return.created_by',$user_register->user_id);
        $this->db->where('return.date >= ',$user_register->date);
        $sq = $this->db->get();
        $this->data['returns'] = $sq->row();

        // echo '<pre>';
        // print_r($user_register);
        // print_r($this->data['cash_in_hand']);
        // print_r($this->data['payments']);
        // print_r($this->data['refund']);
        // print_r($this->data['sales']);
        // print_r($this->data['returns']);

        $this->load->view('v1/admin/views/pos/close_register_new', $this->data);
    ...
    public function close_register_submit($user_id = null)
    {
        ...
        if ($this->form_validation->run() == true) {
            if ($this->Owner || $this->Admin || $this->GP['register-close']) {
                $user_register = $user_id ? $this->pos_model->registerData($user_id) : null;
    ...
    public function open_register()
    {
        ...
        // removed this because of open register not working on cashier
        //$this->sma->checkPermissions('index');
        // removed this because of open register not working on cashier
