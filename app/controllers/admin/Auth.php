...
class Auth extends MY_Controller
{
    ...
    function users()
    {
        ...
        $this->page_construct2('auth/index', $meta, $this->data);
    }
    ...
    public function get_users()
    {
        ...
        foreach ($rows as $row) {
            ...
            $button = '<a href="' . base_url("admin/auth/profile/" . $row->id) . '" class="md-btn md-btn-warning md-btn-wave-light waves-effect waves-button waves-light md-btn-mini">Edit</a>';
            if ($booker) {
                $button .= '<a href="' . base_url("admin/auth/tracking/" . $row->id) . '" class="md-btn md-btn-success md-btn-wave-light waves-effect waves-button waves-light md-btn-mini">Tracking</a>';
                $button .= '<a href="' . base_url("admin/journey_plan/index/" . $row->id) . '" class="md-btn md-btn-success md-btn-wave-light waves-effect waves-button waves-light md-btn-mini" data-mdb-toggle="tooltip" title="">Journey Plan</a>';
                $button .= '<a href="' . base_url("admin/booker_targets/index/" . $row->id) . '" class="md-btn md-btn-success md-btn-wave-light waves-effect waves-button waves-light md-btn-mini" data-mdb-toggle="tooltip" title="">Target Settings</a>';
            }
            if ($deliver) {
                $button .= '<a href="' . base_url("admin/auth/tracking/" . $row->id) . '" class="md-btn md-btn-success md-btn-wave-light waves-effect waves-button waves-light md-btn-mini">Tracking</a>';
                $button .= '<a href="' . base_url("admin/assign_orders/index/" . $row->id) . '" class="md-btn md-btn-success md-btn-wave-light waves-effect waves-button waves-light md-btn-mini" data-mdb-toggle="tooltip" title="">Assign Orders</a>';
            }
            $status = "<span class='uk-badge uk-badge-success' >Active</span>";
            if ($row->active == 0) {
                $status = "<span class='uk-badge uk-badge-danger' >Deactive</span>";
            }
            $data[] = array(
                $row->id,
                $row->first_name,
    ...
    public function runquery_users($onlycoun = "no")
    {
        ...
        $this->db->join('groups as groups', 'groups.id = users.group_id', 'left');
        $i = 0;
        ...
        if (isset($_POST['status'])) {
            if ($_POST['status'] != "all") {
                $this->db->where('users.active', $_POST['status']);
            }
        }
        if (isset($_POST['t'])) {
            if ($_POST['t'] == "4" || $_POST['t'] == "5") {
                $this->db->where('users.group_id', $_POST['t']);
            } else {
                $this->db->where('users.group_id != 4 OR users.group_id != 5');
            }
        } else {
            $this->db->where('users.group_id != 4 OR users.group_id != 5');
        }
    ...
    public function tracking($id)
    {
        ...
        $q = $this->db->get();
        $this->data['trackers'] = $q->result();
        $this->data['error'] = (validation_errors()) ? validation_errors() : $this->session->flashdata('error');
        $bc = [['link' => base_url(), 'page' => lang('home')], ['link' => '#', 'page' => lang('Tracking')]];
        $meta = ['page_title' => lang('User Tracking'), 'bc' => $bc];
        $this->page_construct2('auth/tracking', $meta, $this->data);
    }
    ...
    public function delete_avatar($id = null, $avatar = null)
    {
        if (!$this->ion_auth->logged_in() || !$this->ion_auth->in_group('owner') && $id != $this->session->userdata('user_id')) {
            $this->session->set_flashdata('warning', lang('access_denied'));
            exit("<script type='text/javascript'>setTimeout(function(){ window.top.location.href = '" . $_SERVER['HTTP_REFERER'] . "'; }, 0);</script>");
            redirect($_SERVER['HTTP_REFERER']);
        } else {
    ...
    public function profile($id = null)
    {
        ...
        $this->data['billers'] = $this->site->GetAllSupplierList();
        ...
        $this->page_construct2('auth/profile', $meta, $this->data);
    }
    ...
    public function login($m = null)
    {
        ...
        if ($this->form_validation->run() == true) {
            ...
            if ($this->ion_auth->login($this->input->post('identity'), $this->input->post('password'), $remember, $enteredKey)) {
            ...
            $this->db->select('*');
            $this->db->from('sma_package');
            $this->db->limit(1);
            $query = $this->db->get();
            $package = $query->row();

            $packageStatus = $package->status; // Assuming the package status is stored in the 'status' column
            $endDate = $package->end_date; // Assuming the end date is stored in the 'end_date' column
            $accepted_key = $package->accepted_key;
            $key = $package->encryptionkey;
            // Assuming the end date is stored in the 'end_date' column
            // Assuming the end date is stored in the 'end_date' column

            $showEncryptionKey = ($packageStatus == 1 && $endDate == date('Y-m-d') && $accepted_key == 0) ? true : false;

            $this->data['showEncryptionKey'] = $showEncryptionKey;
    ...
    public function create_user()
    {
        ...
        $this->data['warehouses'] = $this->site->getAllWarehouses();
        $this->data['userslist'] = $this->db->select('id,first_name,last_name')->from('users')->where('active', 1)->get()->result();
        ...
        $this->page_construct2('auth/create_user', $meta, $this->data);
    }
    ...
    public function create_user_submit()
    {
        ...
        if ($this->ion_auth->email_check($email)) {
            $sendvalue['message'] = 'Email already exists in the database.';
        } else {
            $additional_data = [
            ...
            $notify = false; // Set to true if you want to send a notification email

            // Register the new user
            $user_id = $this->ion_auth->register($username, $password, $email, $additional_data, 1, $notify);

            if ($user_id) {
                $sendvalue['status'] = true;
                $sendvalue['message'] = 'User created successfully!';
            } else {
                $sendvalue['message'] = 'Error creating user.';
            }
        }
    ...
    public function edit_user($id = null)
    {
        ...
        $data = [
            ...
            'group_id' => $this->input->post('group'),
            'warehouse_id' => $this->input->post('warehouse'),
            'biller_id' => $this->input->post('biller') == '' ? NULL : $this->input->post('biller'),
            'biller2' => $this->input->post('biller2') == '' ? NULL : $this->input->post('biller2'),
            'biller3' => $this->input->post('biller3') == '' ? NULL : $this->input->post('biller3'),
            'biller4' => $this->input->post('biller4') == '' ? NULL : $this->input->post('biller4'),
            'biller5' => $this->input->post('biller5') == '' ? NULL : $this->input->post('biller5')
        ];
        ...
        if ($this->input->post('password')) {
            ...
            if ($pass != '') {
                if ($pass != $cpass) {
                    $sendvalue['message'] = 'Password not match';
                    echo json_encode($sendvalue);
                    exit;
                }
                $data['password'] = $pass;
        ...
        if ($this->ion_auth->update($user->id, $data)) {
            $sendvalue['message'] = lang('user_updated');
            $sendvalue['status'] = true;
        } else {
            $sendvalue['message'] = 'User update failed';
        }
        echo json_encode($sendvalue);
    }
    ...
    function create_booker()
    {
        ...
        $this->data['supplier'] = $this->site->getAllCompanies('supplier');
        $this->data['warehouses'] = $this->site->getAllWarehouses();
        $this->data['userslist'] = $this->db->select('id,first_name,last_name')->from('users')->where('active', 1)->where('group_id', 3)->get()->result();
        $bc = array(array('link' => admin_url('home'), 'page' => lang('home')), array('link' => admin_url('auth/users?t=3'), 'page' => lang('users')), array('link' => '#', 'page' => lang('Create Booker')));
        $meta = array('page_title' => lang('Bookers'), 'bc' => $bc);
        $this->page_construct2('auth/create_booker', $meta, $this->data);
    }
    public function create_booker_submit()
    {
        $sendvalue['status'] = false;
        $notify = false;
        ...
        $additional_data = array(
            ...
            'biller_id' => $this->input->post('biller'),
            'warehouse_id' => $this->input->post('warehouse'),
            'area_manager_id' => $this->input->post('reportperson'),
            'view_right' => 1,
            'edit_right' => 1,
            'allow_discount' => 0,
        );
        $active = $this->input->post('status');
        $this->ion_auth->register($username, $password, $email, $additional_data, 1, $notify);
        $returnmessage = $this->ion_auth->messages();
        if ($returnmessage == "") {
            $sendvalue['message'] = 'Booker create successfully!';
        } else {
            $sendvalue['message'] = $returnmessage;
        }
        $sendvalue['status'] = true;
        echo json_encode($sendvalue);
    }
    ...
    function create_dispatcher()
    {
        ...
        $this->data['userslist'] = $this->db->select('id,first_name,last_name')->from('users')->where('active', 1)->where('group_id', 3)->get()->result();
        $bc = array(array('link' => admin_url('home'), 'page' => lang('home')), array('link' => admin_url('auth/users?t=3'), 'page' => lang('users')), array('link' => '#', 'page' => lang('Create Booker')));
        $meta = array('page_title' => lang('Dispatchers'), 'bc' => $bc);
        $this->page_construct2('auth/create_dispatcher', $meta, $this->data);
    }
    public function create_dispatcher_submit()
    {
        $sendvalue['status'] = false;
        $notify = false;
        ...
        $returnmessage = $this->ion_auth->messages();
        if ($returnmessage == "") {
            $sendvalue['message'] = 'Dispatcher create successfully!';
        } else {
            $sendvalue['message'] = $returnmessage;
        }
        $sendvalue['status'] = true;
