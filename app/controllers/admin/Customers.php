...
class Customers extends MY_Controller
{
        ...
        if (($name != "" && $formtype == 'pos') || ($sales_type != "" && $name != "" && $company != "")) {
            ...
            if ($q->num_rows() == 0) {
                $data = array(
                    ...
                    'group_id' => '3',
                    'group_name' => 'customer',
                    'etailers_id'  => $etailers
                );
                $this->db->insert('companies', $data);
                $senddata['status'] = true;
                $senddata['message'] = "Customer add successfully!";
            } else {
                $senddata['message'] = "Customer already available";
            }
        } else {
    ...
    {
        $this->db->select('sma_etailers.id,sma_etailers.name,sma_etailers.created_at, CONCAT(users.first_name," ",users.last_name) as created_by');
        $this->db->from('sma_etailers');
        $this->db->join('users as users', 'users.id = sma_etailers.created_by', 'left');
        $this->data['etailers'] = $this->db->get()->result();
        $bc = array(array('link' => base_url(), 'page' => lang('home')), array('link' => '#', 'page' => lang('customers')));
        $meta = array('page_title' => lang('customers'), 'bc' => $bc);
        $this->page_construct2('customers/etaliers', $meta, $this->data);
    }
    public function etailers_add()
    {
       $user = $this->site->getuser();
  
        $etailers_name = $this->input->post('etailers');
        ...
        $this->db->insert('sma_etailers', $data);
        redirect(base_url('admin/customers/etailers'));
    }

    public function etailers_edit($id)
{
