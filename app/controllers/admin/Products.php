...
class Products extends MY_Controller
{
    ...
    public function index()
    {
        ...
        $this->data['formula'] = $this->input->get('formula');
    ...
    public function get_list()
    {
        ...
        // Count Total Rows
        $this->db->select('a.*, b.warehouse_id, c.name AS warehouse_name, b.quantity');
        $this->db->from('products as a');
        // $this->db->join('purchase_items as b','b.product_id = a.id');
        $this->db->join('warehouses_products as b', 'b.product_id = a.id', 'left');
        $this->db->join('warehouses as c', 'c.id = b.warehouse_id', 'left');
        if ($warehouse_id != '') {
            $this->db->where('b.warehouse_id', $warehouse_id);
        }
        if ($supplier != '') {
            $this->db->where('a.supplier1', $supplier);
        }
        if ($brand != '') {
            $this->db->where('a.brand', $warehouse_id);
        }
        if ($category != '') {
        ...
        if ($taxtype != '') {
            $this->db->where('a.tax_method', $taxtype);
        }
        if ($taxtype != '') {
            $this->db->where('a.tax_method', $taxtype);
        }
        ...
        if ($formula != '') {
            $this->db->where('a.formulas', $formula);
        }
        $totalq = $this->db->get();
        $this->runquery('yes');
        $query = $this->db->get();
        ...
        foreach ($rows as $row) {
            ...
            $button .= '<button class="md-btn ';
            if ($row->status == 1) {
                $button .= 'md-btn-danger';
            } else {
                $button .= 'md-btn-primary';
            }
            $button .= ' md-btn-wave-light waves-effect waves-button waves-light md-btn-mini statuschangeproduct" data-id="' . $row->id . '" data-status="' . $row->status . '" >';
            if ($row->status == 1) {
                $button .= 'Deactive';
            } else {
                $button .= 'Active';
            ...
            $cost_tax = 0;
            if ($row->tax_type == 1) {
                $cost_tax = ($row->cost / 100) * $row->tax_rate;
            } elseif ($row->tax_type == 2) {
                $cost_tax = $row->tax_rate;
            }
            $cost_tax = decimalallow($cost_tax);
            $diseases = '';
            if ($row->formula_diseases != 0) {
                $formula_diseases = explode(',', $row->formula_diseases);
                $disease_rows = $this->db->from('sma_diseases')->where_in('id', $formula_diseases)->get()->result();
                foreach ($disease_rows as $key => $disease_row) {
                    if ($key > 0) {
                        $diseases .= ', ';
                    }
                    $diseases .= $disease_row->name;
                }
            }
            ...
            if ($row->image != '' && $row->image != 'no_image.png') {
                $image_url = base_url('uploads/products/' . $row->image);
            }
            ...
            $data[] = [
                ...
                $row->warehouse_name,
                $row->unit,
                decimalallow($row->cost, 4),
                decimalallow($cost_tax, 4),
                decimalallow($row->cost + $cost_tax, 4),
                decimalallow($row->mrp, 4),
                $row->alert_quantity,
                $row->brand,
                $row->category,
                $row->subcategory,
                $row->formula_code,
                $row->prescription == 1 ? 'Yes' : 'No',
                $diseases,
                $row->tax_type == 1 ? 'GST' : '3rd Schedule',
                $row->tax_rate,
                $row->fed_tax,
                $row->company_code,
                '',
    ...
    public function runquery($onlycoun = 'no')
    {
        ...
        if ($onlycoun == 'yes') {
            ...
            $this->db->select('
                ...
                products.code,
                products.name,
                warehouses_products.quantity as total_qty,
                ...
                product_formulas.code as formula_code,
                products.prescription,
                product_formulas.diseases as formula_diseases,
                tax_rates.type as tax_type,
                tax_rates.rate as tax_rate,
                products.fed_tax,
                products.company_code,
                "" as supplier,
                sma_manufacturers.name as manufacturer,
                products.carton_size,
                products.pack_size,
                products.short_expiry_duration,
                products.es_durration,
                IFNULL((
                    SELECT SUM(si.quantity) FROM sma_sale_items as si WHERE si.product_id = products.id
                ), 0) as total_sold,
        ...
        $this->db->join('sma_manufacturers', 'sma_manufacturers.id = products.manufacturer', 'left');
        ...
        $this->db->order_by('warehouses_products.quantity', 'desc');
        $i = 0;
        ...
        if ($supplier != '') {
            $this->db->where('(supplier1 = ' . $supplier . ' OR supplier2 = ' . $supplier . ' OR supplier3 = ' . $supplier . ' OR supplier4 = ' . $supplier . ' OR supplier5 = ' . $supplier . ')');
        }
        ...
        if ($category != '') {
            $this->db->where('(products.category_id = ' . $category . ' OR products.subcategory_id = ' . $category . ')');
        }

        if ($group != '') {
            $this->db->where('products.group_id', $group);
        }

        if ($taxtype != '') {
            $this->db->where('tax_rates.type', $taxtype);
        }

        if ($formula != '') {
            $this->db->where('product_formulas.id', $formula);
        }
    ...
    public function insert_submit()
    {
        ...
        if ($q->num_rows() > 0) {
            ...
            if ($barcode == '') {
                $barcode = date('Y') . '' . date('m') . '' . date('d') . '' . date('H') . '' . date('i') . '' . date('s');
            }
            $insert['code'] = $barcode;
            ...
            $config['file_name'] = date('Y') . '' . date('m') . '' . date('d') . '' . date('H') . '' . date('i') . '' . date('s');
            $this->load->library('upload', $config);
            if ($this->upload->do_upload('image')) {
                $insert['image'] = $this->upload->file_name;
            }
            ...
            $stores = $this->db->select('id,warehouse_id')->from('sma_stores_tb')->where('status', 'active')->get()->result();
            foreach ($stores as $store) {
                $insert2['store_id'] = $store->id;
                $insert2['update_in'] = 'full';
                $insert2['product_id'] = $sendvalue['id'];
                $insert2['product_name'] = $this->input->post('name');
                $insert2['store_product_id'] = '';
                $insert2['update_qty_in'] = 'pack';
                $insert2['price_type'] = 'mrp';
                $insert2['discount'] = 'no';
                $insert2['warehouse_id'] = $store->warehouse_id;
                $insert2['supplier_id'] = $this->input->post('supplier1');
                $insert2['created_by'] = '58';
                $insert2['updated_by'] = '0';
                $insert2['status'] = 'active';
                $this->db->insert('sma_store_products_tb', $insert2);

                $insert3['product_id'] = $sendvalue['id'];
                $insert3['warehouse_id'] = $store->warehouse_id;
                $insert3['store_id'] = $store->id;
                $insert3['type'] = 'Qty Update';
                $insert3['status'] = 'pending';
                $this->db->insert('sma_store_requests_tb', $insert3);
            }
        }
    ...
    public function edit($id = '')
    {
        if ($id != '') {
            ...
            if ($q->num_rows() > 0) {
                ...
                $this->data['manufacturers'] = $this->general_model->GetAllManufacturers();
                $this->data['formulas'] = $this->general_model->GetAllFormulas();
                $bc = [['link' => base_url(), 'page' => lang('home')], ['link' => admin_url('products'), 'page' => lang('products')], ['link' => '#', 'page' => lang('Edit Product')]];
                $meta = ['page_title' => lang('Edit Product'), 'bc' => $bc];
    ...
    public function update_submit()
    {
        ...
        if ($q->num_rows() > 0) {
            ...
            $this->load->library('upload', $config);
            if ($this->upload->do_upload('image')) {
                $set['image'] = $this->upload->file_name;
            }
            $this->db->set($set);
            ...
            // $this->wp->update_product($product_id);
            $stores = $this->db->select('id,warehouse_id')->from('sma_stores_tb')->where('status', 'active')->get()->result();
            foreach ($stores as $store) {
                $insert3['product_id'] = $product_id;
                $insert3['warehouse_id'] = $store->warehouse_id;
                $insert3['store_id'] = $store->id;
                $insert3['type'] = 'Qty Update';
                $insert3['status'] = 'pending';
    ...
    public function view($id = '')
    {
        if ($id != '') {
            ...
            if ($q->num_rows() > 0) {
                $this->data['product'] = $q->result()[0];
                $sid = [];
                if ($this->data['product']->supplier1 != 0) {
                    $sid[] = $this->data['product']->supplier1;
                }
                if ($this->data['product']->supplier2 != 0) {
                    $sid[] = $this->data['product']->supplier2;
                }
                if ($this->data['product']->supplier3 != 0) {
                    $sid[] = $this->data['product']->supplier3;
                }
                if ($this->data['product']->supplier4 != 0) {
                    $sid[] = $this->data['product']->supplier4;
                }
                if ($this->data['product']->supplier5 != 0) {
                    $sid[] = $this->data['product']->supplier5;
                }
                ...
                $this->data['purchases'] = $this->products_model->get_purchases($id);
                $this->data['sales'] = $this->products_model->get_sales($id);

                $this->data['salesreturn'] = $this->products_model->salesreturn($id);
                $this->data['purchasereturn'] = $this->products_model->purchasereturn($id);
                //    echo "<pre>";
                //     print_r($this->data['salesreturn']);
                //    exit;
                $this->data['transfers'] = $this->products_model->get_transfers($id);

                $bc = [['link' => base_url(), 'page' => lang('home')], ['link' => admin_url('products'), 'page' => lang('products')], ['link' => '#', 'page' => lang('View Product')]];
                $meta = ['page_title' => lang('View Product'), 'bc' => $bc];
    ...
    public function delete()
    {
        ...
        if ($this->data['Owner'] || $this->data['Admin'] || $this->data['GP']['products-delete']) {
            if ($reason != '') {
                $this->db->select('id');
    ...
    public function statuschangeproduct()
    {
        $senddata['status'] = false;
        $senddata['message'] = 'Try again!';
        $id = $this->input->post('id');
        ...
        if ($reason != '') {
            ...
            $senddata['status'] = true;
            $senddata['message'] = 'Product status change successfully!';
        } else {
            $senddata['message'] = 'Enter Reason!';
        }
        echo json_encode($senddata);
    ...
    public function groups()
    {
        ...
        $this->data['brands'] = $this->general_model->GetAllBrands();
        $bc = [['link' => base_url(), 'page' => lang('home')], ['link' => '#', 'page' => lang('products')], ['link' => '#', 'page' => lang('Groups')]];
        $meta = ['page_title' => lang('Product Groups'), 'bc' => $bc];
    ...
    public function delete_group()
    {
        ...
        if ($this->data['Owner']) {
            if ($reason != '') {
                ...
                $q = $this->db->get();
                if ($q->num_rows() == 0) {
                    $this->db->delete('product_groups', ['id' => $id]);
                    $senddata['status'] = true;
    ...
    {
        $this->data['error'] = validation_errors() ? validation_errors() : $this->session->flashdata('error');
        $bc = [['link' => base_url(), 'page' => lang('home')], ['link' => '#', 'page' => lang('Formulas')]];
        $meta = ['page_title' => lang('Formulas'), 'bc' => $bc];
    ...
    public function delete_formula()
    {
        ...
        if ($this->data['Owner']) {
            if ($reason != '') {
                ...
                $q = $this->db->get();
                if ($q->num_rows() == 0) {
                    $this->db->delete('product_formulas', ['id' => $id]);
                    $senddata['status'] = true;
    ...
    public function delete_form()
    {
        ...
        if ($this->data['Owner']) {
            if ($reason != '') {
                ...
                $q = $this->db->get();
                if ($q->num_rows() == 0) {
                    $this->db->delete('formula_forms', ['id' => $id]);
                    $senddata['status'] = true;
    ...
    public function formula_strengths()
    {
        $this->data['error'] = validation_errors() ? validation_errors() : $this->session->flashdata('error');
        $bc = [['link' => base_url(), 'page' => lang('home')], ['link' => admin_url('system_settings'), 'page' => lang('system_settings')], ['link' => '#', 'page' => lang('Strengths')]];
        $meta = ['page_title' => lang('Strengths'), 'bc' => $bc];
    ...
    public function delete_strengths()
    {
        ...
        if ($this->data['Owner']) {
            if ($reason != '') {
                ...
                $q = $this->db->get();
                if ($q->num_rows() == 0) {
                    $this->db->delete('formula_strengths', ['id' => $id]);
                    $senddata['status'] = true;
                    $senddata['message'] = 'From delete successfully!';
                } else {
    ...
    public function product_forms()
    {
        $this->data['error'] = validation_errors() ? validation_errors() : $this->session->flashdata('error');
        $bc = [['link' => base_url(), 'page' => lang('home')], ['link' => admin_url('system_settings'), 'page' => lang('system_settings')], ['link' => '#', 'page' => lang('Product Forms')]];
        $meta = ['page_title' => lang('product_forms'), 'bc' => $bc];
    ...
    public function delete_product_form()
    {
        ...
        if ($this->data['Owner']) {
            if ($reason != '') {
                ...
                $q = $this->db->get();
                if ($q->num_rows() == 0) {
                    $this->db->delete('product_forms', ['id' => $id]);
                    $senddata['status'] = true;
                    $senddata['message'] = 'From delete successfully!';
                } else {
    ...
    public function add_old($id = null)
    {
        ...
        if ($this->form_validation->run() == true) {
            ...
            } elseif ($this->input->post('cost') > $this->input->post('mrp')) {
                $this->session->set_flashdata('error', 'Please check cost amount');
                admin_redirect('products/add');
            } elseif ($this->input->post('price') > $this->input->post('mrp')) {
    ...
    public function suggestions()
    {
        ...
        if ($rows) {
            foreach ($rows as $row) {
                $pr[] = ['id' => $row->id, 'label' => $row->name . ' (' . $row->code . ')', 'code' => $row->code, 'name' => $row->name, 'price' => $row->price, 'qty' => 1, 'rate' => $row->rate, 'cost' => $row->cost, 'crossdock' => $row->crossdock, 'dropship' => $row->dropship, 'mrp' => $row->mrp];
            }
    ...
    public function addByAjax()
    {
        ...
        if ($this->input->get('token') && $this->input->get('token') == $this->session->userdata('user_csrf') && $this->input->is_ajax_request()) {
            ...
            if ($this->input->post('cost') > $this->input->post('price')) {
                exit(json_encode(['msg' => lang('Please check cost amount')]));
            } elseif ($this->input->post('cost') > $this->input->post('mrp')) {
                exit(json_encode(['msg' => lang('Please check cost amount')]));
            } elseif ($this->input->post('price') > $this->input->post('mrp')) {
                exit(json_encode(['msg' => lang('Please check MRP amount')]));
            }
