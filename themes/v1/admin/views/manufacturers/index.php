    ...
    <div id="page_content_inner">
        <div class="md-card">
            <div class="md-card-toolbar">
                <h3 class="md-card-toolbar-heading-text">Manufacturers</h3>
    ...
    ?>
        <div class="uk-modal-dialog">
            <div class="uk-modal-header">
                <h3 class="uk-modal-title">Create Manufacturer</h3>
            </div>
...
<div class="uk-modal" id="modal_edit">
    ...
    ?>
        <div class="uk-modal-dialog">
            <div class="uk-modal-header">
                <h3 class="uk-modal-title">Edit Manufacturer</h3>
            </div>
            <div class="uk-modal-body">
                <div class="uk-grid">
                    <div class="uk-width-large-1-1">
                    ...
                    <div class="uk-width-large-1-1">
                        <div class="md-input-wrapper md-input-filled">
                            <label>Phone</label>
                            <input type="text" name="phone" class="md-input md-input-success label-fixed" id="phone" >
                        </div>
                    </div>
                    <div class="uk-width-large-1-1">
                        <div class="md-input-wrapper md-input-filled">
                            <label>Email</label>
                            <input type="text" name="email" class="md-input md-input-success label-fixed" id="email" >
                        </div>
                    </div>
                    <div class="uk-width-large-1-1">
                        <div class="md-input-wrapper md-input-filled">
                            <label>Address</label>
                            <input type="text" name="address" class="md-input md-input-success label-fixed" id="address" >
                        </div>
                    </div>
                </div>
            </div>
...
<script>
    ...
    $.DataTableInit({
        ...
        url:"<?= admin_url('manufacturers/get_manufacturers'); ?>",
        data:data,
        aaSorting: [[1, "desc"]],
        columnDefs: [
            { 
...
<script>
    $(document).ready(function(){
        ...
        $(document).on('click','.editbtn',function(){
            $('#manufacturer_id').val($(this).data('id'));
        ...
        $('#addFrom').submit(function(e){
            ...
            $.ajax({
                url: '<?php echo base_url('admin/manufacturers/insert_manufacturer'); ?>',
                type: 'POST',
        ...
        $('#editFrom').submit(function(e){
            ...
            $('#submitbtn').prop('disabled', true);
            $.ajax({
                url: '<?php echo base_url('admin/manufacturers/update_manufacturer'); ?>',
                type: 'POST',
                data: new FormData(this),
                contentType: false,
                cache: false,
        ...
        $(document).on('click','.deletebtn',function(){
            var id = $(this).data('id');
            Swal.fire({
                title: "Do you want to delete this manufacturers",
                input: "text",
                ...
                if(res.isConfirmed){
                    Swal.fire({
                        title: 'Deleting Own Manufacturers!',
                        showCancelButton: false,
                        didOpen: () => {
                            Swal.showLoading();
                            $.ajax({
                                url: '<?php echo base_url("admin/manufacturers/delete_manufacturers"); ?>',
                                type: 'POST',
