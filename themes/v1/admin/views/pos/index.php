                            ...
                            <div class="uk-width-medium-1-1 uk-row-first">
                                <div class="uk input-group">
                                    <div class="md-input-wrapper">
                                        <?php if($Owner || $Admin):?>
                                            <select name='branches' id="branches" class='md-input'>
                                                <option value=''>All Branches</option>
                                                <?php $warehouse = $this->db->select()->from('warehouses')->get()->result();?>
                                                <?php foreach($warehouse as $wid):?>
                                                    <option value="<?php echo $wid->id; ?>"><?php echo $wid->name; ?></option>
                                                <?php endforeach;?>
                                            </select>                                            
                                        <?php else: ?>
                                            <?php if(!empty($whn)):?>
                                                <?php foreach($whn as $wid):?>
                                                    <h2><?php echo $wid->name;?></h2>
                                                <?php endforeach;?>
                                            <?php else:?>
                                                <h2>All Branches</h2>
                                            <?php endif;?>
                            ...
                            <div class="uk-width-medium-1-1 uk-row-first">
                                <div class="md-input-wrapper" >
                                    <select name="saletype" class="md-input" id="saletype" >
                                        ...
                                        <option value="2" <?php echo $hold_bill[0]->saletype == 2 ? "checked" : "" ?>>Website Sale</option>
    ...
    $(document).ready(function(){
            ...
            $.ajax({
                ...
                success: function (data) {
                    ...
                    if(obj.codestatus){
                        ...
                        //console.log(itemArray[0]['batch']);
                        if(items == null){
                            items = [obj.products];
                            console.log(items[0].batch);
                            ...
                            if (getitems && getitems.length > 0) {
                                var batch = getitems[0].batch;

                                // Check if the batch value already exists in the items array
                                var isDuplicate = getitems.some(item => item.batch === batch);
                                ...
                                if (!isDuplicate) {
                                    ...
                                    console.log('Item already exists:', batch);
                                }
                            } else {
                            ...
                            if (batch) {
                                // Parse the batch data into a variable
                                const parsedBatchData = JSON.parse(batch);
                                
                                // Access the batch properties
                                console.log(parsedBatchData);
                                
                                // You can now use the parsedBatchData variable for further processing
                            } else {
                                console.log('No batch data found in localStorage.');
                                getitems.push(obj.products);
                                localStorage.setItem('pos_items', JSON.stringify(getitems));
                            }                         
        ...
        function selectBatch(pid, wid, piid) {
            $.ajax({
                ...
                if (obj.codestatus) {
                    ...
                    // Check if the batch value already exists in any item in the items array
                    var isDuplicate = getitems.some(item => item.batch === obj.products.batch && item.id === obj.products.id);

                    if (!isDuplicate) {
                        // Add the new item if it's not a duplicate
                        getitems.push(obj.products);
                        console.log('Item added:', obj.products.batch);
                        localStorage.setItem('pos_items', JSON.stringify(getitems));
                    } else {
                        alert('Item already exists:', obj.products.batch);
                    }
                    }
        ...
        function loadProduct(type="category",category=0,brand=0){      
            ...
            $.ajax({                    
                ...
                type: 'GET',
                data: {type:type,category:category,brand:brand,branch:branch},
                success: function(data) {
                    var obj = jQuery.parseJSON(data);
                    if(obj.status){
        ...
        // Update branch value when select#branches changes
        $("#branches").change(function () {
            var branch = $(this).val();
            loadProduct("branch",branch); // Load products with updated branch value
        });
        <?php 
        $user_id = $this->session->userdata('user_id');
        $q = $this->db->select()->from('users')->where('id', $user_id)->get()->row();        
        if($q->warehouse_id != 0):?>            
            var branch = <?php echo $q->warehouse_id ?>;
            loadProduct("branch",branch); 
        ...
        $('#registerclose2').click(function(){
            ...
            $.ajax({
                url: '<?php echo base_url('admin/pos/registerclose_detail_new'); ?>',
                type: 'GET',
                success: function(data) {
                    $('#modal_registerclose > div').html(data);
