            ...
            <div class="md-card-content">
                ...
                <div class="uk-grid" data-uk-grid-margin>
                    ...
                    <div class="uk-width-large-1-1">
                        <div class="md-input-wrapper md-input-filled">
                            <label>Add Products in bulk</label>
                            <input type="file"  id="itesfile" name="itemsfile" class="uk-width-1-1 " >
                        </div>
                    </div>
                    <div class="uk-width-large-1-4" >
                    <div class="md-input-wrapper md-input-filled">
                        <button type="button" class="uk-button uk-button-primary kv-fileinput-upload"><i class="fa fa-upload"></i> Get Items</button>
                    </div>
                    </div>
...
<script>
    $(document).ready(function() {
        ...
        $('#alertQtybtn').click(function() {
            ...
            $.ajax({
                type: 'get',
                url: '<?php echo admin_url('purchases/alertqty'); ?>',
                data: {
...
<script>
       $('#submitFrom').on('click','.kv-fileinput-upload',function(){
            $.ajax({
                url: '<?= admin_url('purchaseorder/bulkuploaditem'); ?>',
                type: 'POST',
                data: new FormData(document.getElementById("submitFrom")),
                contentType: false,
                cache: false,
                processData: false,
                success: function(data){   
                        var items = localStorage.getItem('po_items');
                            if (items == null) {
                                ...
                                var getitems = localStorage.getItem('po_items');
                                getitems.push(data);
                                localStorage.setItem('po_items', getitems);
                            }
                            location.reload();     
                }
