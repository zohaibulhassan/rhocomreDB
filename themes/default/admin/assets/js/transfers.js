  ...
  if (localStorage.getItem("toitems")) {
    ...
    $.each(sortedItems, function () {
      ...
      var product_id = item.row.id,
        ...
        item_batch_qty = item.row.get_selected_product_quantities,
      ...
      } else if (base_quantity > item_batch_qty) {
        $("#row_" + row_no).addClass("danger");
        $("#add_transfer, #edit_transfer").attr("disabled", true);
      }
    });
...
$(document).on("input", ".rquantity", function () {
  ...
  var quantity = parsedObject[item_id].row.qty;
  //console.log(quantity);
  var updateQTY = $(this).val();
  // Update the quantity in the parsed object
  parsedObject[item_id].row.qty = updateQTY;

  // Convert the updated object back to a string and store it in localStorage
  localStorage.setItem('toitems', JSON.stringify(parsedObject));
});
