...
$(document).ready(function (e) {
  ...
  $(document).on("click", "#editItem", function () {
    ...
    (slitems[item_id].row.fup = 1),
      ...
      customer_type == "consignment" || customer_type == "cost"
        ? ((slitems[item_id].row.get_selected_product_price = price),
          (slitems[item_id].row.consignment = parseFloat(price)))
        : customer_type == "dropship"
        ? ((slitems[item_id].row.get_selected_product_dropship = price),
          (slitems[item_id].row.dropship = parseFloat(price)))
        : customer_type == "crossdock"
        ? ((slitems[item_id].row.get_selected_product_crossdock = price),
          (slitems[item_id].row.crossdock = parseFloat(price)))
        : customer_type == "services"
        ? "0"
        : "0";
    ...
    (slitems[item_id].row.unit = unit),
      ...
      (slitems[item_id].row.discount = $("#pdiscount").val()
        ? $("#pdiscount").val()
        : ""),
  ...
  $(document).on("click", "#sellGiftCard", function (e) {
    if (count == 1) {
      slitems = {};
      if ($("#slwarehouse").val() && $("#customer_select").val()) {
        // $("#customer_select").select2("readonly", true);
  ...
  $(document).on(
    "change",
    ".discount_one, .discount_two, .discount_three",
    function () {
      var row = $(this).closest("tr");
      item_id = row.attr("data-item-id");
      ...
      if ($(this)[0].checked == true) {
        ...
        tax_value = formatMoney(
          (parseFloat(slitems[item_id].row.cost) *
            parseFloat(slitems[item_id].row.base_quantity) *
            parseFloat(slitems[item_id].tax_rate.rate)) /
            100
        );
        ...
        discount_with_qty +=
          total_discount_one * parseFloat(slitems[item_id].row.base_quantity);
        slitems[item_id].row.discount = JSON.stringify(
          discount_with_qty + old_discount
        );
      } else {
        ...
        discount_with_qty +=
          total_discount_one * parseFloat(slitems[item_id].row.base_quantity);
        slitems[item_id].row.discount = JSON.stringify(
          old_discount - discount_with_qty
        );
      }
  ...
  /* --------------------------
     * Edit Row Batch Number Method
     -------------------------- */
  $(document)
    .on("focus", ".rbatch_number", function () {})
    .on("change", ".rbatch_number", function () {
      var row = $(this).closest("tr");
  ...
  var old_row_qty;
  $(document)
    .on("focus", ".rprod_expiry", function () {
  ...
  $(document)
    ...
    .on("change", ".rquantity", function () {
      ...
      let get_selected_product_quantities = Math.trunc(
        $(
          "#product_batch_balance_quantity_" + item_id + "_" + select_index
        ).val()
      );
      ...
      // console.log(get_selected_product_quantities);
      //  console.log(new_qty);

      // console.log($("#batch_remain_quantity_" + item_id + "_" + select_index).val());
  ...
  $(document).on("click", "#removeReadonly", function () {
    // $("#customer_select").select2("readonly", false);
...
function loadItems() {
  if (localStorage.getItem("slitems")) {
    ...
    $.each(sortedItems, function () {
      ...
      // var check_discount_list = ((item.check_discount_list == null) ? "" : item.check_discount_list.split(","));
      var check_discount_list = item.check_discount_list;
      // console.log((item.check_discount_list).length);
      ...
      var product_batch_balance_quantity =
        item.row.product_batch_balance_quantity == null
          ? ""
          : item.row.product_batch_balance_quantity.split(",");
      ...
      unit_price = formatDecimal(
        (customer_type == "consignment" || customer_type == "cost")
          ? formatMoney(item.row.get_selected_product_price)
          : customer_type == "dropship"
          ? formatMoney(item.row.get_selected_product_dropship)
          : customer_type == "crossdock"
      ...
      item_price =
        item_tax_method == 0
          ? formatDecimal(price_change - pr_tax_val, 4)
      ...
      tr_html =
        ...
        item_type +
        '"><input name="company_code[]" type="hidden" class="rcode" value="' +
        company_code +
      ...
      if (site.settings.product_serial == 1) {
        tr_html +=
          '<td class="text-right"><input class="form-control input-sm rserial" name="serial[]" type="text" autocomplete="off" id="serial_' +
          row_no +
          '" value="' +
          item_serial +
          '"></td>';
      ...
      // Check inclusive and exclusive

      if (item.row.tax_method == "0") {
        // If inclusive
        console.log('CT: '+customer_type);
        price_value_condition =
          (customer_type == "consignment" || customer_type == "cost")
            ? item.row.get_selected_product_price
            : customer_type == "dropship"
            ...
            ? "services"
            : "0";
          console.log('PVC: '+price_value_condition);

        inclusive_price_calculation =
          price_value_condition / (item.tax_rate.rate / 100 + 1);
        ...
        tr_html +=
          ...
          '"><input id="ruprice_' +
          row_no +
          '" class="ruprice" name="unit_price[]" type="hidden" value="' +
          inclusive_price_calculation +
          '"><input id="realuprice_' +
          row_no +
          '" class="realuprice" name="real_unit_price[]" type="hidden" value="' +
          inclusive_price_calculation +
          '"><span class="text-right sprice" id="sprice_' +
          row_no +
          '">' +
          formatMoney(inclusive_price_calculation) +
          "</span> </td>";
      }

      if (item.row.tax_method == "1") {
        // If exclusive
        console.log('CT: '+customer_type);
        price_value_condition =
        (customer_type == "consignment" || customer_type == "cost")
        ...
        tr_html +=
          ...
          row_no +
          '">' +
          formatMoney(price_value_condition) +
          "</span> </td>";
      }
      ...
      if (select_batch == "empty" || select_batch == null) {
        ...
        for (
          ...
          tr_html +=
            ...
            '">';
          tr_html +=
            '<input id="product_batch_balance_quantity_' +
            row_no +
            "_" +
            index_select +
            '"  name="product_batch_balance_quantity" type="hidden" class="runit" value="' +
            product_batch_balance_quantity[index_select] +
            '">';
      ...
      tr_html +=
        '<td> <input id="discount_one_' +
        row_no +
        '"  class="discount_one" name="discount_one_' +
        my_count +
        '" ' +
        (JSON.parse(item.row.discount_one_checked) == true ? "checked" : "") +
        '  type="checkbox" value="' +
        (item.row.discount_one ? item.row.discount_one.slice(0, 5) : 0) +
        '" onClick="this.select();"  data-id="' +
        row_no +
        '" data-item="' +
        item_id +
        '" > ( ' +
        ...
        "% ) <br> " +
        formatMoney(
          ((item.row.get_selected_product_price * item.row.discount_one) /
            100) *
            /* get_selected_product_price */ item.row.base_quantity
        ) +
        "  </td>";
      ...
      tr_html +=
        '<td><input id="discount_two_' +
        row_no +
        '"  class="discount_two" name="discount_two_' +
        my_count +
        '" ' +
        (JSON.parse(item.row.discount_two_checked) == true ? "checked" : "") +
        ' type="checkbox" value="' +
        (item.row.discount_two ? item.row.discount_two.slice(0, 5) : 0) +
        '" onClick="this.select();"  data-id="' +
        row_no +
        '" data-item="' +
        item_id +
        ...
        "% ) <br> " +
        formatMoney(
          ((item.row.get_selected_product_price * item.row.discount_two) /
            100) *
            /* get_selected_product_price */ item.row.base_quantity
        ) +
        "  </td>";
      ...
      tr_html +=
        '<td><input id="discount_three_' +
        row_no +
        '"  class="discount_three" name="discount_three_' +
        my_count +
        '" ' +
        (JSON.parse(item.row.discount_three_checked) == true ? "checked" : "") +
        ' type="checkbox" value="' +
        (item.row.discount_three ? item.row.discount_three.slice(0, 5) : 0) +
        '" onClick="this.select();"  data-id="' +
        row_no +
        '" data-item="' +
        item_id +
        ...
        "% ) <br> " +
        formatMoney(
          ((item.row.get_selected_product_price * item.row.discount_three) /
            100) *
            /* get_selected_product_price */ item.row.base_quantity
        ) +
        "  </td>";
      ...
      tr_html +=
        '<td><select id="check_discount_list_' +
        row_no +
        '" name="check_discount_list[]" class="check_discount_list form-control" data-item-id="' +
        row_no +
        '">';
      tr_html += '<option value="">Select</option>';
      for (
        let index_select = 0;
        index_select < check_discount_list.length;
        index_select++
      ) {
        tr_html +=
          "<option " +
          (item.row.discount_three ==
          check_discount_list[index_select].percentage
            ? "selected"
            : " ") +
          ' value="' +
          check_discount_list[index_select].percentage +
          '">' +
          check_discount_list[index_select].discount_code +
          "</option>";
      }
      tr_html += "</select>";
      for (
        ...
        tr_html +=
          '<input id="discount_percentage_' +
          row_no +
          "_" +
          index_select +
          '" name="discount_percentage" type="hidden" class="runit" value="' +
          check_discount_list[index_select].percentage +
          '">';
      }
      tr_html += "</td>";
      ...
      tr_html +=
        '<td>   <span class="text-right fed_tax" id="fed_tax_' +
        row_no +
        '">' +
        (item.row.get_selected_fed_tax_rate
          ? item.row.get_selected_fed_tax_rate
          : 0) +
        '</span>         <input id="fed_tax_' +
        row_no +
        '"  class="form-control input-sm text-right fed_tax" name="fed_tax_' +
        my_count +
      ...
      if (
        (site.settings.product_discount == 1 && allow_discount == 1) ||
        item_discount
      ) {
        tr_html +=
          ...
          row_no +
          '">' +
          /* formatMoney(0 - (item_discount * item_qty))*/ formatMoney(
            0 - item_ds
          ) +
          "</span></td>";
      }
      ...
      if (customer_type == "consignment" || customer_type == "cost") {
        if (site.settings.tax1 == 1) {
          ...
          tr_html +=
            ...
            '" value="' +
            pr_tax.id +
            '"> <input class="form-control input-sm text-right rproduct_tax" name="product_tax_' +
            my_count +
            '" type="hidden" id="product_tax_' +
            my_count +
            '" value="' +
            ...
            row_no +
            '">' +
            (parseFloat(pr_tax_rate) != 0
              ? "(" + formatDecimal(pr_tax_rate) + ")"
              : "") +
            " " +
            formatMoney(pr_tax_val * item_qty) +
            "</span></td>";
        ...
        if (site.settings.tax1 == 1) {
          tr_html +=
            '<td class="text-right"><input class="form-control input-sm text-right rproduct_tax" name="product_tax[]" type="hidden" id="product_tax_' +
            row_no +
            '" value="0"><span class="text-center sproduct_tax" id="sproduct_tax_' +
            row_no +
            '" style="display: block;"> 0 </span></td>';
        }
      }


        tr_html +=
          '<td class="text-right"><span class="text-right advtax" id="advtax' +
          row_no +
          '">';
          var advatax = 0;
          if(item.gst_no == ""){
            advatax = ((parseFloat(price_value_condition)+pr_tax_val)*parseFloat(item.row.qty))/100*item.row.adv_tax_nonreg;
          } else{
            advatax = ((parseFloat(price_value_condition)+pr_tax_val)*parseFloat(item.row.qty))/100*item.row.adv_tax_reg;
          }
          tr_html += advatax.toFixed(4);
        tr_html += "</span><input type='hidden' name='advtax[]' value='"+advatax.toFixed(4)+"' ></td>";
        console.log(adv_tax_total);
        adv_tax_total = adv_tax_total+ advatax;
        console.log(adv_tax_total);
        ...
        if (item.gst_no == "") {
          if (customer_type == "consignment" || customer_type == "cost") {
            if (item.tax_rate.type == 1) {
              ...
              tr_html +=
                '<td class="text-right"><input class="form-control input-sm text-right rproduct_tax" name="further_tax_' +
                my_count +
                '" type="hidden" id="further_tax_' +
                row_no +
                '" value="' +
                $furhter_tax_value +
                '"><span class="text-right sproduct_tax" id="sproduct_tax_' +
                ...
                "(" +
                formatDecimal(
                  (item.row.further_tax * 100) /
                    ((parseFloat(item_price) +
                      ((customer_type == "consignment" || customer_type == "cost")
                        ? parseFloat(pr_tax_val)
                        : 0)) *
                      parseFloat(item_qty) -
                      item_ds)
                ) +
                ")" +
                "<br> " +
                formatMoney($furhter_tax_value) +
                "</span></td>";
            } else {
              tr_html +=
                '<td class="text-right"><span class="text-right sproduct_tax" > 0 </span></td>';
            }
          } else {
            tr_html +=
              '<td class="text-right"><span class="text-right sproduct_tax" > 0 </span></td>';
        ...
        if (item.gst_no == "") {
          if (customer_type == "consignment" || customer_type == "cost") {
            if (item.tax_rate.type == 1) {
              check_further_tax = true;
              $furhter_tax_value =
                ((item.row.get_selected_product_price * item.row.further_tax) /
                  100) *
                item.row.base_quantity;
              $furhter_tax_value_total += $furhter_tax_value;
              tr_html +=
                '<td class="text-right"><input class="form-control input-sm text-right rproduct_tax" name="further_tax_' +
                my_count +
                ...
                '">' +
                "(" +
                formatDecimal(item.row.further_tax) +
                ")" +
                "<br> " +
                formatMoney($furhter_tax_value) +
                "</span></td>";
      ...
      formatMoney(
        (parseFloat(item_price) +
          ((customer_type == "consignment" || customer_type == "cost") ? parseFloat(pr_tax_val) : 0)) *
          parseFloat(item_qty) -
          item_ds +
          (item.gst_no == "" &&
          (customer_type == "consignment" || customer_type == "cost") &&
          item.tax_rate.type == 1
            ? parseFloat($furhter_tax_value)
            : 0)
      );

      // console.log(inclusive_price_calculation);

      if (inclusive_price_calculation == 0) {
        // alert(1);
        ...
        sub_total =
          (parseFloat(item_price) +
            ...
            parseFloat(item_qty) -
          item_ds +
          parseFloat(item.row.get_selected_fed_tax_rate * item_qty) +
          advatax +
          (item.gst_no == "" &&
          (customer_type == "consignment" || customer_type == "cost") &&
          item.tax_rate.type == 1
        ...
        sub_total =
          ...
          item.tax_rate.type == 1
            ? (inclusive_price_calculation +
                ((customer_type == "consignment" || customer_type == "cost")
                  ? parseFloat(item.row.get_selected_fed_tax_rate) +
                    parseFloat(pr_tax_val) +
                    parseFloat($furhter_tax_value / item_qty)
                  : 0)) *
              parseFloat(item_qty)
            : (inclusive_price_calculation +
                ((customer_type == "consignment" || customer_type == "cost")
    ...
    var tfoot =
      ...
      '</th><th class="text-center"></th><th class="text-center"></th><th class="text-center"></th><th class="text-center"></th><th class="text-center"></th><th class="text-center"></th><th class="text-center"></th><th class="text-center"></th><th class="text-center"></th>';
    if (
      (site.settings.product_discount == 1 && allow_discount == 1) ||
      product_discount
    ) {
      tfoot +=
        '<th class="text-right">' + formatMoney(product_discount) + "</th>";
    }
    ...
    tfoot += '<th class="text-center"> '+adv_tax_total.toFixed(4)+' </th>';
    // alert(check_further_tax);
    if (check_further_tax == true) {
      tfoot +=
        '<th class="text-center"> ' +
        formatMoney($furhter_tax_value_total) +
        " </th>";
    } else {
      tfoot += '<th class="text-center"> 0 </th>';
    }
...
$(document).on("change", ".check_discount_list", function () {
  ...
  //     tr_html += '<option ' + ((item.row.get_selected_batch_code == check_discount_list[index_select].id) ? "selected" : " ") + ' value="' + check_discount_list[index_select].id + '">' + check_discount_list[index_select].discount_code + '</option>';
  // }
  ...
  // alert(get_selected_check_discount_list);

  slitems[item_id].row.discount_three = get_selected_check_discount_list;
...
$(document).on("change", ".batch_number", function () {
  console.log(123);
  // Add
  let warehouse_select = $("#s2id_slwarehouse #select2-chosen-3").text();

  // Edit
  let warehouse_select_2 = $("#s2id_slwarehouse #select2-chosen-2").text();

  let warehouse_id = $('#slwarehouse').val();

  /* if (warehouse_select === "Warehouse 1" || warehouse_select_2 === "Warehouse 1") {
  //   warehouse_id = 1;
  // }
  // else if ( warehouse_select === "Warehouse 2" || warehouse_select_2 === "Warehouse 2") {
  //   warehouse_id = 2;
  // }
  // else {
  //   warehouse_id = 0;
  ...
  //$('#s2id_slwarehouse').find(':selected');
  ...
  $.ajax({
    ...
    data: {
      ...
      get_warehouse_id: warehouse_id,
      customer_id: $("input.hidden_customer_id").val(),
    },
    success: function (data) {
      console.log(data);
      ...
      get_selected_product_quantities11 = "123";
